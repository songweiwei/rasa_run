...
def webToBot():
    ...
    result=response.text.encode('ascii').decode('unicode_escape')
    # print(result)
    # result=json.dumps(result, ensure_ascii=False)
    # print(json.loads(response.text.encode('ascii').decode('unicode_escape')))
    # print(type(json.loads(response.text)[0]))
    # print(type(json.loads(response.text)[0]["text"]))
    reply={}
    reply["reply"]=json.loads(result,strict=False)[0]["text"]
